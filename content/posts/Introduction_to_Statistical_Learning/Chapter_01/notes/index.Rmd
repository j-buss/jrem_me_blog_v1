...
- Transformation matrix: A matrix which contains in columns where the basis vectors end at. For example if the normal matrix describing $\hat{i}$ and $\hat{j}$ is: $\begin{bmatrix}1&0\\0&1\end{bmatrix}$ then after a transformation by rotating by 90$\deg$ counterclockwise would be $\begin{bmatrix}0&-1\\1&0\end{bmatrix}$
...
- How can we summarize a set of linear equations? 
$$
\begin{split}
